...
Otherwise, you need to twist the following scripts and change the command lines 
...
If you cannot wait, run the following commands one by one. Otherwise, skip to "Prerequsites".
	...
	cd test-run
	# Please modify allIDs.txt,run.config and sample.annotation.txt 
...
================================
!!!
1)  allIDs.txt --sample ID file, an unique ID per line. 
...
###########################################################
#
## output files. Most are self-expanatory.
#
###########################################################
